...
*/
  ...
  #ifdef RTT_CB_ALIGN
    #error "Custom RTT_CB_ALIGN() is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
  #ifdef RTT_BUFFER_ALIGN
    #error "Custom RTT_BUFFER_ALIGN() is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
  #ifdef RTT_PUT_CB_SECTION
    #error "Custom RTT_PUT_CB_SECTION() is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
  #ifdef RTT_PUT_BUFFER_SECTION
    #error "Custom RTT_PUT_BUFFER_SECTION() is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
  #ifdef RTT_BUFFER_ALIGNMENT
    #error "Custom RTT_BUFFER_ALIGNMENT is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
  #ifdef RTT_ALIGNMENT
    #error "Custom RTT_ALIGNMENT is not supported for RTT_CPU_CACHE_LINE_SIZE != 0"
  #endif
#endif
  ...
  #define RTT_MAX_NUM_UP_BUFFERS                          2    // Number of up-buffers (T->H) available on this target
#endif

#ifndef   RTT_MAX_NUM_DOWN_BUFFERS
  #define RTT_MAX_NUM_DOWN_BUFFERS                        2    // Number of down-buffers (H->T) available on this target
#endif

#ifndef RTT_BUFFER_SECTION
  #if defined(RTT_SECTION)
    #define RTT_BUFFER_SECTION RTT_SECTION
  #endif
#endif

#ifndef   RTT_ALIGNMENT
  #define RTT_ALIGNMENT                                   RTT_CPU_CACHE_LINE_SIZE
#endif

#ifndef   RTT_BUFFER_ALIGNMENT
  #define RTT_BUFFER_ALIGNMENT                            RTT_CPU_CACHE_LINE_SIZE
#endif

#ifndef   RTT_MODE_DEFAULT
  #define RTT_MODE_DEFAULT                                RTT_MODE_NO_BLOCK_SKIP
#endif
  ...
  #define RTT_MEMCPY_USE_BYTELOOP                         0
#endif
...
#if RTT_ALIGNMENT || RTT_BUFFER_ALIGNMENT
  #if (defined __GNUC__)
    #define RTT_ALIGN(Var, Alignment) Var __attribute__ ((aligned (Alignment)))
  #elif (defined __ICCARM__) || (defined __ICCRX__)
    #define PRAGMA(A) _Pragma(#A)
#define RTT_ALIGN(Var, Alignment) RTT_PRAGMA(data_alignment=Alignment) \
                                  Var
  ...
  #endif
#else
  #define RTT_ALIGN(Var, Alignment) Var
#endif

#if defined(RTT_SECTION) || defined (RTT_BUFFER_SECTION)
  #if (defined __GNUC__)
    #define RTT_PUT_SECTION(Var, Section) __attribute__ ((section (Section))) Var
  #elif (defined __ICCARM__) || (defined __ICCRX__)
#define RTT_PUT_SECTION(Var, Section) RTT_PRAGMA(location=Section) \
                                        Var
  #elif (defined __CC_ARM)
    #define RTT_PUT_SECTION(Var, Section) __attribute__ ((section (Section), zero_init))  Var
  #else
    #error "Section placement not supported for this compiler."
  #endif
#else
  #define RTT_PUT_SECTION(Var, Section) Var
#endif

#if RTT_ALIGNMENT
  #define RTT_CB_ALIGN(Var)  RTT_ALIGN(Var, RTT_ALIGNMENT)
#else
  #define RTT_CB_ALIGN(Var)  Var
#endif

#if RTT_BUFFER_ALIGNMENT
  #define RTT_BUFFER_ALIGN(Var)  RTT_ALIGN(Var, RTT_BUFFER_ALIGNMENT)
#else
  #define RTT_BUFFER_ALIGN(Var)  Var
#endif


#if defined(RTT_SECTION)
  #define RTT_PUT_CB_SECTION(Var) RTT_PUT_SECTION(Var, RTT_SECTION)
#else
  #define RTT_PUT_CB_SECTION(Var) Var
#endif

#if defined(RTT_BUFFER_SECTION)
  #define RTT_PUT_BUFFER_SECTION(Var) RTT_PUT_SECTION(Var, RTT_BUFFER_SECTION)
#else
  #define RTT_PUT_BUFFER_SECTION(Var) Var
#endif
...
*/
#define INIT()  {                                                                                    \
                  volatile RTT_CB* pRTTCBInit;                                                       \
                  pRTTCBInit = (volatile RTT_CB*)((char*)&_RTT + RTT_UNCACHED_OFF);                  \
                  do {                                                                               \
...
static void _DoInit(void) {
  volatile RTT_CB* p;   // Volatile to make sure that compiler cannot change the order of accesses to the control block
  //
  // Initialize control block
  //
  p                     = (volatile RTT_CB*)((char*)&_RTT + RTT_UNCACHED_OFF);  // Access control block uncached so that nothing in the cache ever becomes dirty and all changes are visible in HW directly
...
*
*  Function description
*    Stores a specified number of characters in RTT ring buffer
...
*    NumBytes     Number of bytes to be stored in the RTT control block.
*
*  Return value
*    >= 0 - Number of bytes written into buffer.
*/
static unsigned _WriteBlocking(RTT_BUFFER_UP* pRing, const char* pBuffer, unsigned NumBytes) {
  ...
  do {
    ...
    pDst = (pRing->pBuffer + WrOff) + RTT_UNCACHED_OFF;
#if RTT_MEMCPY_USE_BYTELOOP
...
#else
    RTT_MEMCPY((void*)pDst, pBuffer, NumBytesToWrite);
    NumBytesWritten += NumBytesToWrite;
...
*/
static void _WriteNoCheck(RTT_BUFFER_UP* pRing, const char* pData, unsigned NumBytes) {
  ...
  if (Rem > NumBytes) {
    ...
    pRing->WrOff = WrOff;
#else
    RTT_MEMCPY((void*)pDst, pData, NumBytes);
    RTT__DMB();                     // Force data write to be complete before writing the <WrOff>, in case CPU is allowed to change the order of memory accesses
    ...
    pDst = pRing->pBuffer + RTT_UNCACHED_OFF;
    ...
    NumBytesAtOnce = Rem;
    pDst = (pRing->pBuffer + WrOff) + RTT_UNCACHED_OFF;
    RTT_MEMCPY((void*)pDst, pData, NumBytesAtOnce);
    NumBytesAtOnce = NumBytes - Rem;
    pDst = pRing->pBuffer + RTT_UNCACHED_OFF;
    RTT_MEMCPY((void*)pDst, pData + Rem, NumBytesAtOnce);
    RTT__DMB();                     // Force data write to be complete before writing the <WrOff>, in case CPU is allowed to change the order of memory accesses
...
*/
static void _PostTerminalSwitch(RTT_BUFFER_UP* pRing, unsigned char TerminalId) {
  unsigned char ac[2];
...
*/
static unsigned _GetAvailWriteSpace(RTT_BUFFER_UP* pRing) {
  unsigned RdOff;
  unsigned WrOff;
  unsigned r;
  //
...
/*********************************************************************
*
*       RTT_ReadUpBufferNoLock()
*
*  Function description
*    Reads characters from real-time-terminal control block
...
*
*  Parameters
*    Channel  Index of Up-buffer to be used.
...
unsigned RTT_ReadUpBufferNoLock(unsigned Channel, void* pData, unsigned BufferSize) {
  ...
  INIT();
  pRing = (RTT_BUFFER_UP*)((char*)&_RTT.aUp[Channel] + RTT_UNCACHED_OFF);  // Access uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  ...
  if (RdOff > WrOff) {
    ...
    pSrc = (pRing->pBuffer + RdOff) + RTT_UNCACHED_OFF;
#if RTT_MEMCPY_USE_BYTELOOP
...
#else
    RTT_MEMCPY(pBuffer, (void*)pSrc, NumBytesRem);
    NumBytesRead += NumBytesRem;
    pBuffer      += NumBytesRem;
    BufferSize   -= NumBytesRem;
    RdOff        += NumBytesRem;
...
*
*  Parameters
*    Channel  Index of Down-buffer to be used (e.g. 0 for "Terminal").
...
unsigned RTT_ReadNoLock(unsigned Channel, void* pData, unsigned BufferSize) {
  ...
  unsigned                WrOff;
  unsigned char*          pBuffer;
  RTT_BUFFER_DOWN* pRing;
  volatile char*          pSrc;
  //
  INIT();
  pRing = (RTT_BUFFER_DOWN*)((char*)&_RTT.aDown[Channel] + RTT_UNCACHED_OFF);  // Access uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  pBuffer = (unsigned char*)pData;
  RdOff = pRing->RdOff;
  WrOff = pRing->WrOff;
  NumBytesRead = 0u;
...
*    call RTT_ReadUpBufferNoLock() instead.
*/
unsigned RTT_ReadUpBuffer(unsigned Channel, void* pBuffer, unsigned BufferSize) {
  ...
  //
  NumBytesRead = RTT_ReadUpBufferNoLock(Channel, pBuffer, BufferSize);
  //
  // Finish up.
  //
  RTT_UNLOCK();
...
unsigned RTT_Read(unsigned Channel, void* pBuffer, unsigned BufferSize) {
  ...
  // Call the non-locking read function
  //
  NumBytesRead = RTT_ReadNoLock(Channel, pBuffer, BufferSize);
  //
  // Finish up.
  //
  RTT_UNLOCK();
...
/*********************************************************************
*
*       RTT_WriteWithOverwriteNoLock
*
*  Function description
*    Stores a specified number of characters in RTT
*    control block.
*    RTT_WriteWithOverwriteNoLock does not lock the application 
*    and overwrites data if the data does not fit into the buffer.
*
*  Parameters
*    Channel      Index of "Up"-buffer to be used (e.g. 0 for "Terminal").
*    pBuffer      Pointer to character array. Does not need to point to a \0 terminated string.
*    NumBytes     Number of bytes to be stored in the RTT control block.
*
*  Notes
...
*        Either by calling RTT_Init() or calling another RTT API function first.
*    (3) Do not use RTT_WriteWithOverwriteNoLock if a J-Link 
...
void RTT_WriteWithOverwriteNoLock(unsigned Channel, const void* pBuffer, unsigned NumBytes) {
  ...
  do {
    if (Avail > NumBytes) {
      ...
      // Last round
      //
      pDst = (pRing->pBuffer + pRing->WrOff) + RTT_UNCACHED_OFF;
#if RTT_MEMCPY_USE_BYTELOOP
      Avail = NumBytes;
      while (NumBytes--) {
        *pDst++ = *pData++;
      ...
      pRing->WrOff = 0;
#else
      RTT_MEMCPY((void*)pDst, pData, Avail);
      pData += Avail;
      RTT__DMB();                     // Force data write to be complete before writing the <WrOff>, in case CPU is allowed to change the order of memory accesses
      pRing->WrOff = 0;
      NumBytes -= Avail;
...
*    RTT_WriteSkipNoLock does not lock the application and
...
/*********************************************************************
*
*       RTT_WriteDownBufferNoLock
*
*  Function description
*    Stores a specified number of characters in RTT
*    control block inside a <Down> buffer.
*    RTT_WriteDownBufferNoLock does not lock the application.
*    Used to do the same operation that J-Link does, to transfer 
*    RTT data from other channels, such as TCP/IP or UART.
*
*  Parameters
*    Channel  Index of "Down"-buffer to be used.
*    pBuffer      Pointer to character array. Does not need to point to a \0 terminated string.
*    NumBytes     Number of bytes to be stored in the RTT control block.
*
*  Return value
...
unsigned RTT_WriteDownBufferNoLock(unsigned Channel, const void* pBuffer, unsigned NumBytes) {
  ...
  //
  pData = (const char *)pBuffer;
  pRing = (RTT_BUFFER_UP*)((char*)&_RTT.aDown[Channel] + RTT_UNCACHED_OFF);  // Access uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  //
  // How we output depends upon the mode...
  //
  switch (pRing->Flags) {
  case RTT_MODE_NO_BLOCK_SKIP:
    //
  ...
  case RTT_MODE_NO_BLOCK_TRIM:
    ...
    break;
  case RTT_MODE_BLOCK_IF_FIFO_FULL:
    //
...
*    Stores a specified number of characters in RTT
*    control block which is then read by the host.
*    RTT_WriteNoLock does not lock the application.
*
*  Parameters
*    Channel  Index of "Up"-buffer to be used (e.g. 0 for "Terminal").
*    pBuffer      Pointer to character array. Does not need to point to a \0 terminated string.
...
*
*  Function description
*    Stores a specified number of characters in RTT control block in a <Down> buffer.
*
*  Parameters
*    Channel  Index of "Up"-buffer to be used (e.g. 0 for "Terminal").
*    pBuffer      Pointer to character array. Does not need to point to a \0 terminated string.
...
*    call RTT_WriteDownBufferNoLock() instead.
*/
unsigned RTT_WriteDownBuffer(unsigned Channel, const void* pBuffer, unsigned NumBytes) {
  ...
  INIT();
  RTT_LOCK();
  Status = RTT_WriteDownBufferNoLock(Channel, pBuffer, NumBytes);  // Call the non-locking write function
  RTT_UNLOCK();
  return Status;
}
...
unsigned RTT_Write(unsigned Channel, const void* pBuffer, unsigned NumBytes) {
  ...
  INIT();
  RTT_LOCK();
  Status = RTT_WriteNoLock(Channel, pBuffer, NumBytes);  // Call the non-locking write function
  RTT_UNLOCK();
  return Status;
}
...
*
*  Function description
*    Stores string in RTT control block.
...
unsigned RTT_WriteString(unsigned Channel, const char* s) {
  ...
  Len = STRLEN(s);
  return RTT_Write(Channel, s, Len);
}

/*********************************************************************
*
*       RTT_PutCharSkipNoLock
*
*  Function description
*    Stores a single character/byte in RTT buffer.
*    RTT_PutCharSkipNoLock does not lock the application and
...
unsigned RTT_PutChar(unsigned Channel, char c) {
  ...
  //
  if (pRing->Flags == RTT_MODE_BLOCK_IF_FIFO_FULL) {
...
*
*  Function description
*    Reads one character from the RTT buffer.
...
*
*  Function description
*    Waits until at least one character is avaible in the RTT buffer.
...
*
*  Function description
*    Checks if at least one character for reading is available in the RTT buffer.
*
*  Return value
...
int RTT_HasKey(void) {
  ...
  INIT();
  pRing = (RTT_BUFFER_DOWN*)((char*)&_RTT.aDown[0] + RTT_UNCACHED_OFF);  // Access uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  RdOff = pRing->RdOff;
...
int RTT_AllocDownBuffer(const char* sName, void* pBuffer, unsigned BufferSize, unsigned Flags) {
  int Channel;
  volatile RTT_CB* pRTTCB;

  INIT();
  RTT_LOCK();
  pRTTCB = (volatile RTT_CB*)((unsigned char*)&_RTT + RTT_UNCACHED_OFF);  // Access RTTCB uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  Channel = 0;
  do {
    if (pRTTCB->aDown[Channel].pBuffer == NULL) {
      break;
    }
    Channel++;
  } while (Channel < pRTTCB->MaxNumDownBuffers);
  if (Channel < pRTTCB->MaxNumDownBuffers) {
    pRTTCB->aDown[Channel].sName        = sName;
    pRTTCB->aDown[Channel].pBuffer      = (char*)pBuffer;
    pRTTCB->aDown[Channel].SizeOfBuffer = BufferSize;
    pRTTCB->aDown[Channel].RdOff        = 0u;
    pRTTCB->aDown[Channel].WrOff        = 0u;
    pRTTCB->aDown[Channel].Flags        = Flags;
    RTT__DMB();                     // Force data write to be complete before writing the <WrOff>, in case CPU is allowed to change the order of memory accesses
  } else {
    Channel = -1;
  }
...
int RTT_AllocUpBuffer(const char* sName, void* pBuffer, unsigned BufferSize, unsigned Flags) {
  ...
  Channel = 0;
  do {
    if (pRTTCB->aUp[Channel].pBuffer == NULL) {
      break;
    }
    Channel++;
  } while (Channel < pRTTCB->MaxNumUpBuffers);
  if (Channel < pRTTCB->MaxNumUpBuffers) {
    pRTTCB->aUp[Channel].sName        = sName;
    pRTTCB->aUp[Channel].pBuffer      = (char*)pBuffer;
    pRTTCB->aUp[Channel].SizeOfBuffer = BufferSize;
    pRTTCB->aUp[Channel].RdOff        = 0u;
    pRTTCB->aUp[Channel].WrOff        = 0u;
    pRTTCB->aUp[Channel].Flags        = Flags;
    RTT__DMB();                     // Force data write to be complete before writing the <WrOff>, in case CPU is allowed to change the order of memory accesses
  } else {
    Channel = -1;
  }
...
*
*  Parameters
*    Channel      Index of the buffer to configure.
*    sName        Pointer to a constant name string.
*    pBuffer      Pointer to a buffer to be used.
*    BufferSize   Size of the buffer.
*    Flags        Operating modes. Define behavior if buffer is full (not enough space for entire message).
...
int RTT_ConfigUpBuffer(unsigned Channel, const char* sName, void* pBuffer, unsigned BufferSize, unsigned Flags) {
  ...
  INIT();
  pRTTCB = (volatile RTT_CB*)((unsigned char*)&_RTT + RTT_UNCACHED_OFF);  // Access RTTCB uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  if (Channel < (unsigned)pRTTCB->MaxNumUpBuffers) {
    RTT_LOCK();
    if (Channel > 0u) {
      pRTTCB->aUp[Channel].sName        = sName;
      pRTTCB->aUp[Channel].pBuffer      = (char*)pBuffer;
...
int RTT_ConfigDownBuffer(unsigned Channel, const char* sName, void* pBuffer, unsigned BufferSize, unsigned Flags) {
  ...
  INIT();
  pRTTCB = (volatile RTT_CB*)((unsigned char*)&_RTT + RTT_UNCACHED_OFF);  // Access RTTCB uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  if (Channel < (unsigned)pRTTCB->MaxNumDownBuffers) {
    RTT_LOCK();
    if (Channel > 0u) {
      pRTTCB->aDown[Channel].sName        = sName;
      pRTTCB->aDown[Channel].pBuffer      = (char*)pBuffer;
...
*
*  Parameters
*    Channel      Index of the buffer to renamed.
*    sName        Pointer to a constant name string.
*
*  Return value
*    >= 0  O.K.
...
int RTT_SetTerminal (unsigned char TerminalId) {
  ...
  if (TerminalId < sizeof(_aTerminalId)) { // We only support a certain number of channels
    ...
    pRing = (RTT_BUFFER_UP*)((char*)&_RTT.aUp[0] + RTT_UNCACHED_OFF);  // Access uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
    RTT_LOCK();                     // Lock to make sure that no other task is writing into buffer, while we are and number of free bytes in buffer does not change downwards after checking and before writing
    if ((pRing->Flags & RTT_MODE_MASK) == RTT_MODE_BLOCK_IF_FIFO_FULL) {
...
int RTT_TerminalOut (unsigned char TerminalId, const char* s) {
  ...
  if (TerminalId < (char)sizeof(_aTerminalId)) { // We only support a certain number of channels
    ...
    RTT_LOCK();
    Avail = _GetAvailWriteSpace(pRing);
    switch (pRing->Flags & RTT_MODE_MASK) {
    case RTT_MODE_NO_BLOCK_SKIP:
      //
      // If we are in skip mode and there is no space for the whole
...
unsigned RTT_GetBytesInBuffer(unsigned Channel) {
  ...
  //
  pRTTCB = (volatile RTT_CB*)((unsigned char*)&_RTT + RTT_UNCACHED_OFF);  // Access RTTCB uncached to make sure we see changes made by the J-Link side and all of our changes go into HW directly
  RdOff = pRTTCB->aUp[Channel].RdOff;
  WrOff = pRTTCB->aUp[Channel].WrOff;
  if (RdOff <= WrOff) {
    r = WrOff - RdOff;
  } else {
    r = pRTTCB->aUp[Channel].SizeOfBuffer - (WrOff - RdOff);
  }
  return r;
}
